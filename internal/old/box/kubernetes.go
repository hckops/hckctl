...
func NewKubeBox(template *client2.BoxV1) *LocalKubeBox {
	l := logger.With().Str("provider", "kube").Logger()

	box, err := client2.NewOutOfClusterKubeBox(
		template,
		&client2.ResourceOptions{
...
func (local *LocalKubeBox) Open() {
	...
	local.box.OnTunnelCallback = func(port client2.PortV1) {
